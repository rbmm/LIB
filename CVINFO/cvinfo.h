...
enum LEAF_ENUM_e : unsigned short {
...
union CV_modifier_t {
	...
	struct {
		...
		unsigned short  MOD_unused      :12;
		unsigned short  MOD_static      :1;
	};
};
...
enum
{
	prop_packed      = 0x0001,     // true if structure is packed
	prop_ctor        = 0x0002,     // true if constructors or destructors present
	prop_ovlops      = 0x0004,     // true if overloaded operators present
	prop_isnested    = 0x0008,     // true if this is a nested class
	prop_cnested     = 0x0010,     // true if this class contains nested types
	prop_opassign    = 0x0020,     // true if overloaded assignment (=)
	prop_opcast      = 0x0040,     // true if casting methods
	prop_fwdref      = 0x0080,     // true if forward reference (incomplete defn)
	prop_scoped      = 0x0100,     // scoped definition
	prop_hasuniquename = 0x0200,   // true if there is a decorated name following the regular name
	prop_sealed      = 0x0400,     // true if class cannot be used as a base class
	prop_hfa         = 0x1800,     // CV_HFA_e
	prop_intrinsic   = 0x2000,     // true if class is an intrinsic type (e.g. __m128d)
	prop_mocom       = 0xc000     // CV_MOCOM_UDT_e
};
...
struct lfPointer : public lfPointerBody {
	union {
		struct {
...
typedef struct lfClass_16t {
    LEAF_ENUM_e   leaf;           // LF_CLASS_16t, LF_STRUCT_16t
...
struct lfClass {
    LEAF_ENUM_e   leaf;           // LF_CLASS, LF_STRUCT, LF_INTERFACE
    unsigned short  count;          // count of number of elements in class
    CV_prop_t       property;       // property attribute field (prop_t)
    CV_typ_t        field;          // type index of LF_FIELD descriptor list
    CV_typ_t        derived;        // type index of derived from list if not zero
    CV_typ_t        vshape;         // type index of vshape table for this class
    lfNumeric		length_name;    // data describing length of structure in
                                    // bytes and name
};
...
struct lfUnion {
    LEAF_ENUM_e   leaf;           // LF_UNION
    unsigned short  count;          // count of number of elements in class
    CV_prop_t       property;       // property attribute field
    CV_typ_t        field;          // type index of LF_FIELD descriptor list
	
	lfNumeric length_name;			// variable length data describing length of
                                    // structure and name
};
...
typedef struct lfEnum_16t {
    LEAF_ENUM_e   leaf;           // LF_ENUM_16t
    unsigned short  count;          // count of number of elements in class
    CV_typ16_t      utype;          // underlying type of the enum
    CV_typ16_t      field;          // type index of LF_FIELD descriptor list
    CV_prop_t       property;       // property attribute field
    char   name[];        // length prefixed name of enum
} lfEnum_16t;

typedef struct lfEnum {
    LEAF_ENUM_e   leaf;           // LF_ENUM
    unsigned short  count;          // count of number of elements in class
    CV_prop_t       property;       // property attribute field
...
typedef struct lfProc_16t {
    LEAF_ENUM_e   leaf;           // LF_PROCEDURE_16t
...
typedef struct lfProc {
    LEAF_ENUM_e   leaf;           // LF_PROCEDURE
    CV_typ_t        rvtype;         // type index of return value
    CV_call_e   calltype;       // calling convention (CV_call_t)
    CV_funcattr_t   funcattr;       // attributes
    unsigned short  parmcount;      // number of parameters
...
typedef struct lfMFunc_16t {
    LEAF_ENUM_e   leaf;           // LF_MFUNCTION_16t
    CV_typ16_t      rvtype;         // type index of return value
...
typedef struct lfArgList_16t {
    LEAF_ENUM_e   leaf;           // LF_ARGLIST_16t
    unsigned short  count;          // number of arguments
    CV_typ16_t      arg[CV_ZEROLEN];      // number of arguments
} lfArgList_16t;

typedef struct lfArgList {
    LEAF_ENUM_e   leaf;           // LF_ARGLIST, LF_SUBSTR_LIST
...
typedef struct lfFieldList_16t {
    LEAF_ENUM_e   leaf;           // LF_FIELDLIST_16t
    char            data[CV_ZEROLEN];         // field list sub lists
} lfFieldList_16t;


typedef struct lfFieldList {
    LEAF_ENUM_e   leaf;           // LF_FIELDLIST
    unsigned char            data[];         // field list sub lists
} lfFieldList;
...
typedef struct lfMethodList {
    unsigned short leaf;
    mlMethod  mList[];         // really a mlMethod type
} lfMethodList;
...
typedef struct lfBitfield_16t {
    LEAF_ENUM_e   leaf;           // LF_BITFIELD_16t
...
typedef struct lfDimCon_16t {
    LEAF_ENUM_e   leaf;           // LF_DIMCONU_16t or LF_DIMCONLU_16t
...
typedef struct lfDimCon {
    LEAF_ENUM_e   leaf;           // LF_DIMCONU or LF_DIMCONLU
...
typedef struct lfDimVar_16t {
    LEAF_ENUM_e   leaf;           // LF_DIMVARU_16t or LF_DIMVARLU_16t
    unsigned short  rank;           // number of dimensions
    CV_typ16_t      typ;            // type of index
...
typedef struct lfDimVar {
    LEAF_ENUM_e   leaf;           // LF_DIMVARU or LF_DIMVARLU
...
typedef struct lfUShort {
    LEAF_ENUM_e   leaf;           // LF_unsigned short
...
typedef struct lfCmplx128 {
    LEAF_ENUM_e   leaf;           // LF_COMPLEX128
...
struct lfBClass {
    LEAF_ENUM_e   leaf;           // LF_BCLASS, LF_BINTERFACE
    CV_fldattr_t    attr;           // attribute
    CV_typ_t        index;          // type index of base class
    lfNumeric		offset;			// variable length offset of base within class
};
...
struct lfVBClass {
    LEAF_ENUM_e   leaf;           // LF_VBCLASS | LV_IVBCLASS
    CV_fldattr_t    attr;           // attribute
    CV_typ_t        index;          // type index of direct virtual base class
    CV_typ_t        vbptr;          // type index of virtual base pointer
    lfNumeric		vbpoff;         // virtual base pointer offset from address point
                                    // followed by virtual base offset from vbtable

	bool get_end(long* vbp, long* off, char** ppEnd)
	{
		lfNumeric* p;
		return vbpoff.value(vbp, (char**)&p) && p->value(off, ppEnd);
	}
};
...
typedef struct lfFriendFcn_16t {
    LEAF_ENUM_e   leaf;           // LF_FRIENDFCN_16t
...
typedef struct lfMember_16t {
    LEAF_ENUM_e   leaf;           // LF_MEMBER_16t
...
struct lfMember {
    LEAF_ENUM_e   leaf;           // LF_MEMBER
    CV_fldattr_t    attr;           // attribute mask
    CV_typ_t        index;          // index of type record for field
	lfNumeric		offset_name;    // variable length offset of field followed
                                    // by length prefixed name of field
};
...
struct lfSTMember_16t {
    LEAF_ENUM_e   leaf;           // LF_STMEMBER_16t
    CV_typ16_t      index;          // index of type record for field
    CV_fldattr_t    attr;           // attribute mask
	char			name[];			// length prefixed name of field
};

struct lfSTMember {
    LEAF_ENUM_e   leaf;           // LF_STMEMBER
    CV_fldattr_t    attr;           // attribute mask
    CV_typ_t        index;          // index of type record for field
    char			name[];			// length prefixed name of field
};
...
struct lfEnumerate {
    ...
    lfNumeric		value_name; // variable length value field followed
...
struct lfMemberModify {
    ...
    char   name[];    // length prefixed member name
};
...
typedef enum SYM_ENUM_e : unsigned short {
...
struct SYM_HEADER {
	...
	SYM_HEADER* NextSym() {
		return (SYM_HEADER*)RtlOffsetToPointer(&type, len);
	}
};
...
struct MANYREGSYM : SYM_HEADER {// S_MANYREG
    CV_typ_t        typind;     // Type index or metadata token
    unsigned char   count;      // count of number of registers
    unsigned char   reg[1];     // count register enumerates followed by
                                // length-prefixed name.  Registers are
...
struct MANYREGSYM2 : SYM_HEADER {// S_MANYREG2
    CV_typ_t        typind;     // Type index or metadata token
...
struct ATTRMANYREGSYM : SYM_HEADER {// S_MANMANYREG
    CV_typ_t        typind;     // Type index or metadata token
    CV_lvar_attr    attr;       // local var attributes
    unsigned char   count;      // count of number of registers
    unsigned char   reg[1];     // count register enumerates followed by
                                // length-prefixed name.  Registers are
                                // most significant first.
    char   name[];   // utf-8 encoded zero terminate name
};

struct ATTRMANYREGSYM2 : SYM_HEADER {// S_MANMANYREG2 | S_ATTR_MANYREG
    CV_typ_t        typind;     // Type index or metadata token
    CV_lvar_attr    attr;       // local var attributes
    unsigned short  count;      // count of number of registers
    unsigned short  reg[1];     // count register enumerates followed by
...
struct CONSTSYM : SYM_HEADER {// S_CONSTANT or S_MANCONSTANT
...
struct UDTSYM : SYM_HEADER {// S_UDT | S_COBOLUDT
    CV_typ_t        typind;     // Type index
    unsigned char   name[1];    // Length-prefixed name
};

struct MANTYPREF : SYM_HEADER {// S_MANTYPREF
    CV_typ_t        typind;     // Type index
};

struct SEARCHSYM : SYM_HEADER {// S_SSEARCH
    unsigned long   startsym;   // offset of the procedure
    unsigned short  seg;        // segment of symbol
};


struct CFLAGSYM : SYM_HEADER {// S_COMPILE
...
struct COMPILESYM : SYM_HEADER {// S_COMPILE2
    struct {
...
struct COMPILESYM3 : SYM_HEADER {// S_COMPILE3
    struct {
        unsigned long   iLanguage       :  8;   // language index
        unsigned long   fEC             :  1;   // compiled for E/C
        unsigned long   fNoDbgInfo      :  1;   // not compiled with debug info
...
struct ENVBLOCKSYM : SYM_HEADER {// S_ENVBLOCK
    struct {
        unsigned char  rev              : 1;    // reserved
        unsigned char  pad              : 7;    // reserved, must be 0
    } flags;
    char   rgsz[];    // Sequence of zero-terminated strings
};

struct OBJNAMESYM : SYM_HEADER {// S_OBJNAME
    unsigned long   signature;  // signature
    unsigned char   name[1];    // Length-prefixed name
};


struct ENDARGSYM : SYM_HEADER {// S_ENDARG
};


struct RETURNSYM : SYM_HEADER {// S_RETURN
...
struct ENTRYTHISSYM : SYM_HEADER {// S_ENTRYTHIS
...
struct BPRELSYM32 : SYM_HEADER {
    CV_off32_t      off;        // BP-relative offset
    CV_typ_t        typind;     // Type index or Metadata token
    unsigned char   name[1];    // Length-prefixed name
};

struct FRAMERELSYM : SYM_HEADER {// S_MANFRAMEREL | S_ATTR_FRAMEREL
...
struct SLOTSYM32 : SYM_HEADER {// S_LOCALSLOT or S_PARAMSLOT
    unsigned long   iSlot;      // slot index
    CV_typ_t        typind;     // Type index or Metadata token
    unsigned char   name[1];    // Length-prefixed name
};

struct ATTRSLOTSYM : SYM_HEADER { // S_MANSLOT
    unsigned long   iSlot;      // slot index
    CV_typ_t        typind;     // Type index or Metadata token
    CV_lvar_attr    attr;       // local var attributes
    unsigned char   name[1];    // Length-prefixed name
...
struct ANNOTATIONSYM : SYM_HEADER {// S_ANNOTATION
    CV_uoff32_t     off;
    unsigned short  seg;
...
struct DATASYM32 : SYM_HEADER {// S_LDATA32, S_GDATA32, S_LMANDATA, S_GMANDATA
...
struct DATASYMHLSL : SYM_HEADER {// S_GDATA_HLSL, S_LDATA_HLSL
    CV_typ_t        typind;     // Type index
...
struct DATASYMHLSL32 : SYM_HEADER {// S_GDATA_HLSL32, S_LDATA_HLSL32
    CV_typ_t        typind;     // Type index
...
struct DATASYMHLSL32_EX : SYM_HEADER {// S_GDATA_HLSL32_EX, S_LDATA_HLSL32_EX
    CV_typ_t        typind;     // Type index
...
struct PUBSYM32 : SYM_HEADER {// S_PUB32
...
struct MANPROCSYM : SYM_HEADER {// S_GMANPROC, S_LMANPROC, S_GMANPROCIA64 or S_LMANPROCIA64
    unsigned long   pParent;    // pointer to the parent
    unsigned long   pEnd;       // pointer to this blocks end
    unsigned long   pNext;      // pointer to next symbol
    unsigned long   len;        // Proc length
    ...
    char   name[];    // optional name field
};

struct MANPROCSYMMIPS : SYM_HEADER {// S_GMANPROCMIPS or S_LMANPROCMIPS
    unsigned long   pParent;    // pointer to the parent
    unsigned long   pEnd;       // pointer to this blocks end
    unsigned long   pNext;      // pointer to next symbol
    unsigned long   len;        // Proc length
...
struct THUNKSYM32 : SYM_HEADER {// S_THUNK32
    unsigned long   pParent;    // pointer to the parent
    unsigned long   pEnd;       // pointer to this blocks end
    unsigned long   pNext;      // pointer to next symbol
    CV_uoff32_t     off;
    ...
    unsigned char   name[1];    // Length-prefixed name
    unsigned char   variant[]; // variant portion of thunk
};
...
struct TRAMPOLINESYM : SYM_HEADER {  // S_TRAMPOLINE// Trampoline thunk symbol
...
struct LABELSYM32 : SYM_HEADER {// S_LABEL32
    CV_uoff32_t     off;
    unsigned short  seg;
...
struct BLOCKSYM32 : SYM_HEADER {// S_BLOCK32
    unsigned long   pParent;    // pointer to the parent
    unsigned long   pEnd;       // pointer to this blocks end
...
struct WITHSYM32 : SYM_HEADER {// S_WITH32
    unsigned long   pParent;    // pointer to the parent
    unsigned long   pEnd;       // pointer to this blocks end
    unsigned long   len;        // Block length
    CV_uoff32_t     off;        // Offset in code segment
...
struct CEXMSYM32 : SYM_HEADER {// S_CEXMODEL32
...
struct VPATHSYM32 : SYM_HEADER {// S_VFTABLE32
...
struct REGREL32 : SYM_HEADER {// S_REGREL32
    CV_uoff32_t     off;        // offset of symbol
    CV_typ_t        typind;     // Type index or metadata token
...
struct ATTRREGREL : SYM_HEADER {// S_MANREGREL | S_ATTR_REGREL
    CV_uoff32_t     off;        // offset of symbol
    CV_typ_t        typind;     // Type index or metadata token
    unsigned short  reg;        // register index for symbol
    CV_lvar_attr    attr;       // local var attributes
...
struct THREADSYM32 : SYM_HEADER {// S_LTHREAD32 | S_GTHREAD32
    CV_typ_t        typind;     // type index
...
struct SLINK32 : SYM_HEADER {// S_SLINK32
...
struct PROCSYMMIPS : SYM_HEADER {// S_GPROCMIPS or S_LPROCMIPS
    unsigned long   pParent;    // pointer to the parent
    unsigned long   pEnd;       // pointer to this blocks end
    unsigned long   pNext;      // pointer to next symbol
    unsigned long   len;        // Proc length
...
struct PROCSYMIA64 : SYM_HEADER {// S_GPROCIA64 or S_LPROCIA64
    unsigned long   pParent;    // pointer to the parent
    unsigned long   pEnd;       // pointer to this blocks end
    unsigned long   pNext;      // pointer to next symbol
    unsigned long   len;        // Proc length
...
struct REFSYM : SYM_HEADER {// S_PROCREF_ST, S_DATAREF_ST, or S_LPROCREF_ST
...
struct ALIGNSYM : SYM_HEADER {// S_ALIGN
};

struct OEMSYMBOL : SYM_HEADER {// S_OEM
    unsigned char   idOem[16];  // an oem ID (GUID)
    CV_typ_t        typind;     // Type index
...
struct PROCSYM : SYM_HEADER {// S_GPROC16 or S_LPROC16
    unsigned long   pParent;    // pointer to the parent
    unsigned long   pEnd;       // pointer to this blocks end
    unsigned long   pNext;      // pointer to next symbol
};
...
struct THUNKSYM : SYM_HEADER {// S_THUNK
    unsigned long   pParent;    // pointer to the parent
    unsigned long   pEnd;       // pointer to this blocks end
    unsigned long   pNext;      // pointer to next symbol
};

struct BLOCKSYM : SYM_HEADER {// S_BLOCK16
    unsigned long   pParent;    // pointer to the parent
    unsigned long   pEnd;       // pointer to this blocks end
};


struct WITHSYM : SYM_HEADER {// S_WITH16
    unsigned long   pParent;    // pointer to the parent
    unsigned long   pEnd;       // pointer to this blocks end
};

struct FRAMEPROCSYM : SYM_HEADER { // S_FRAMEPROC
...
struct UNAMESPACE : SYM_HEADER {// S_UNAMESPACE
    char   name[];    // name
};

struct SEPCODESYM : SYM_HEADER {// S_SEPCODE
    unsigned long   pParent;    // pointer to the parent
...
struct BUILDINFOSYM : SYM_HEADER {// S_BUILDINFO
    CV_ItemId       id;         // CV_ItemId of Build Info.
};

struct INLINESITESYM : SYM_HEADER {// S_INLINESITE
    unsigned long   pParent;   // pointer to the inliner
    unsigned long   pEnd;      // pointer to this block's end
    CV_ItemId       inlinee;   // CV_ItemId of inlinee
    char   binaryAnnotations[];   // an array of compressed binary annotations.
};

struct INLINESITESYM2 : SYM_HEADER {// S_INLINESITE2
    unsigned long   pParent;        // pointer to the inliner
    unsigned long   pEnd;           // pointer to this block's end
    CV_ItemId       inlinee;        // CV_ItemId of inlinee
...
struct LOCALSYM : SYM_HEADER {// S_LOCAL
    CV_typ_t        typind;     // type index   
    CV_LVARFLAGS    flags;      // local var flags

    char   name[];   // Name of this symbol, a null terminated array of UTF8 characters.
};

struct FILESTATICSYM : SYM_HEADER {// S_FILESTATIC
    CV_typ_t        typind;     // type index   
...
struct DEFRANGESYM : SYM_HEADER { // S_DEFRANGE   // A live range of sub field of variable
...
struct DEFRANGESYMSUBFIELD : SYM_HEADER { // S_DEFRANGE_SUBFIELD// A live range of sub field of variable. like locala.i
    CV_uoff32_t     program;    // DIA program to evaluate the value of the symbol
...
struct DEFRANGESYMREGISTER : SYM_HEADER { // S_DEFRANGE_REGISTER    // A live range of en-registed variable
...
struct DEFRANGESYMFRAMEPOINTERREL : SYM_HEADER { // S_DEFRANGE_FRAMEPOINTER_REL   // A live range of frame variable
...
struct DEFRANGESYMFRAMEPOINTERREL_FULL_SCOPE : SYM_HEADER { // S_DEFRANGE_FRAMEPOINTER_REL// A frame variable valid in all function scope 
...
struct DEFRANGESYMSUBFIELDREGISTER : SYM_HEADER {// S_DEFRANGE_SUBFIELD_REGISTER // A live range of sub field of variable. like locala.i
...
struct DEFRANGESYMREGISTERREL : SYM_HEADER { // S_DEFRANGE_REGISTER_REL   // A live range of variable related to a register.
...
struct DEFRANGESYMHLSL : SYM_HEADER {// S_DEFRANGE_HLSL or S_DEFRANGE_DPC_PTR_TAG    // A live range of variable related to a symbol in HLSL code.
    ...
    CV_LVAR_ADDR_RANGE range;               // Range of addresses where this program is valid
    unsigned char   data[];       // variable length data specifying gaps where the value is not available
...
struct LOCALDPCGROUPSHAREDSYM : SYM_HEADER {// S_LOCAL_DPC_GROUPSHARED
    CV_typ_t        typind;     // type index   
    CV_LVARFLAGS    flags;      // local var flags
...
struct DPCSYMTAGMAP : SYM_HEADER {// S_DPC_SYM_TAG_MAP   // A map for DPC pointer tag values to symbol records.
...
struct FUNCTIONLIST : SYM_HEADER {// S_CALLERS or S_CALLEES
...
struct POGOINFO : SYM_HEADER {// S_POGODATA
...
struct ARMSWITCHTABLE : SYM_HEADER {// S_ARMSWITCHTABLE
...
struct MODTYPEREF : SYM_HEADER {// S_MOD_TYPEREF
...
struct SECTIONSYM : SYM_HEADER {// S_SECTION
...
struct COFFGROUPSYM : SYM_HEADER {// S_COFFGROUP
...
struct EXPORTSYM : SYM_HEADER {// S_EXPORT
...
struct CALLSITEINFO : SYM_HEADER {// S_CALLSITEINFO
...
struct HEAPALLOCSITE : SYM_HEADER {// S_HEAPALLOCSITE
    CV_off32_t      off;                // offset of call site
    unsigned short  sect;               // section index of call site
...
struct FRAMECOOKIE : SYM_HEADER {// S_FRAMECOOKIE
    CV_off32_t      off;                // Frame relative offset
...
struct DISCARDEDSYM : SYM_HEADER {// S_DISCARDED
...
struct REFMINIPDB : SYM_HEADER {// S_REF_MINIPDB
    union {
    ...
    char   name[];            // zero terminated name string
};

struct PDBMAP : SYM_HEADER { // S_PDBMAP
    char   name[];   // zero terminated source PDB filename followed by zero
...
struct CV_DebugSLinesFileBlockHeader_t {
	CV_off32_t     offFile; // offset into array of CV_DebugSFile[] in DEBUG_S_FILECHKSMS section 
	CV_off32_t     nLines;
	CV_off32_t     cbBlock;
...
union lfRecord
{
	...
	lfVBClass		VBClass;   // LF_VBCLASS
	lfIndex			Index;	   // LF_INDEX	
};
...
#pragma pack(pop)
	...
	TYPTYPE* Next() {
		return (TYPTYPE*)RtlOffsetToPointer(&u, len);
	}
