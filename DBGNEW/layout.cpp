...
BOOL CUILayot::CreateLayout(HWND hwndParent)
{
	HWND hwnd = GetWindow(hwndParent, GW_CHILD);

	while (hwnd)
	{
		if (!AddChild(hwndParent, hwnd)) return FALSE;
...
BOOL CUILayot::AddChild(HWND hwndParent, HWND hwnd)
{
	RECT rc, rcParent;
	...
	if (rcParent.right - rc.left < rc.left)
	{
		f |= movAx;
	}

	if (rcParent.right - rc.right < rc.right)
	{
		f |= movBx;
	}

	if (rcParent.bottom - rc.top < rc.top)
	{
		f |= movAy;
	}

	if (rcParent.bottom - rc.bottom < rc.bottom)
	{
		f |= movBy;
	}
