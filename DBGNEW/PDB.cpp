...
NTSTATUS PdbReader::Init(PdbFileHeader* header, SIZE_T ViewSize, PGUID signature)
{
	...
	return *signature == psh.signature ? STATUS_SUCCESS : STATUS_REVISION_MISMATCH;
}
...
NTSTATUS OpenPdbFromSymbols(PHANDLE hFile, 
				 PCWSTR PdbFileName, 
				 PCWSTR NtSymbolPath, 
				 PGUID Signature, 
				 ULONG Age)
{
	...
	PWSTR FilePath = (PWSTR)alloca((wcslen(NtSymbolPath) + 2*wcslen(PdbFileName) + 44) * sizeof(WCHAR));
				 ...
				 BOOL& bUserChoice)
{
	...
	if (!bUserChoice)
	{
		*pbSystem = FALSE;
		...
		if (0 <= (status = OpenPdbFromSymbols(hFile, PdbFileName, NtSymbolPath, Signature, Age)))
		{
			*pbSystem = TRUE;
			return STATUS_SUCCESS;
		}

		// try load from same folder as pe file
		if (PePath && (c = wcsrchr(PePath, '\\')))
		{
			ULONG subSize = RtlPointerToOffset(PePath, ++c);
			ULONG cbPdb = (ULONG)(1 + wcslen(PdbFileName)) * sizeof(WCHAR);
			PWSTR FilePath = (PWSTR)alloca(subSize + cbPdb);
			memcpy(FilePath, PePath, subSize);
			memcpy(RtlOffsetToPointer(FilePath, subSize), PdbFileName, cbPdb);

			if (0 <= (status = (*PePath == '\\' ? OpenNtFile : OpenDosFile)(hFile, FilePath)))
			{
				return STATUS_SUCCESS;
			}
		}
	...
	*hFile = 0;
	...
	{
		INT_PTR r = dlg->DoModal((HINSTANCE)&__ImageBase, MAKEINTRESOURCE(IDD_DIALOG17), ZGLOBALS::getMainHWND(), 0);
...
NTSTATUS PdbReader::Open(CV_INFO_PDB* lpcvh, PCWSTR NtSymbolPath, PCWSTR PePath, PBOOL pbSystemPath)
{
...
__loop:
	HANDLE hFile;
	NTSTATUS status = OpenPdb(&hFile, FileName, PePath, 
		NtSymbolPath, &lpcvh->Signature, lpcvh->Age, pbSystemPath, bUserChoice);
