...
void _lineHeap::Compact()
{
	if (SIZE_T size = (SIZE_T)PAGE_ALIGN(m_size))
	{
		PVOID pv = RtlOffsetToPointer(m_buf, m_TotalSize - size);
		if (0 <= ZwFreeVirtualMemory(NtCurrentProcess(), &pv, &size, MEM_RELEASE))
		{
			m_TotalSize = RtlPointerToOffset(m_buf, pv);
		}
	}
}
