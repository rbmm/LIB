...
class WINZ_API CDocHost : public IDispatch,
	IServiceProvider,
	IOleClientSite,
	IOleInPlaceSite, 
	IDocHostUIHandler,
...
public:
	...
	BOOL CreateHost(HWND hwnd);
	HRESULT put_URL(PWSTR v);
	void Navigate(PCWSTR url, PCWSTR ref = 0);
	void _Navigate(BSTR url, BSTR ref = 0);
	void DestroyHost();
...
private:
	...
	virtual ITypeInfo* GetTypeInfo(PVOID* ppvInstance = 0);
	virtual PCWSTR GetUserAgent();
};

class WINZ_API ZWebFrame : public ZWnd
{
	CDocHost* _pDoc;
protected:
	virtual CDocHost* createDocHost();

	virtual LRESULT WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam);
public:
	void Navigate(PWSTR url, PWSTR ref = 0);
